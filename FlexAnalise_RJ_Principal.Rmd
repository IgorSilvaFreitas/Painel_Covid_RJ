...
download.file(url = "http://sistemas.saude.rj.gov.br/tabnetbd/csv/pni_covid16352870872.xlsx", 
...
download.file(url = "http://sistemas.saude.rj.gov.br/tabnetbd/csv/pni_covid16352870963.xlsx", 
              destfile = temp, mode="wb")
...
download.file(url = "http://sistemas.saude.rj.gov.br/tabnetbd/csv/pni_covid16352870975.xlsx", 
              destfile = temp, mode="wb")
...
#RJ_Total = BaseFinalRJ
RJ_Total = RJ


#table(RJ_Total$Municipios)
RJ_Total <- RJ_Total |> dplyr::filter(ibgeID!=33)
municipios |> dplyr::rename(ibeID=populacao)
municipios$ibgeID <- as.numeric(municipios$ibgeID)
RJ_Total <- left_join(RJ_Total, municipios, by="ibgeID")
...
RJ_Total$date <- ymd(RJ_Total$date)
aux_data = RJ_Total |> 
  distinct(date)
...
RJ_Total = left_join(RJ_Total,MM_aux_geral_casos, by = c("date", "city"))

RJ_Total = left_join(RJ_Total,MM_aux_geral_obitos, by = c("date", "city"))

RJ_Total = left_join(RJ_Total,MM_aux_geral_vacina, by = c("date", "city"))

RJ_Total = left_join(RJ_Total,MM_aux_geral_vacina2, by = c("date", "city"))

RJ_Total = left_join(RJ_Total,MM_aux_geral_ocup_Cli, by = c("date", "city"))

RJ_Total = left_join(RJ_Total,MM_aux_geral_ocup_Uti, by = c("date", "city"))
...
#save(RJ_Total, file="RJ_Total.rdata")
...
RJ_Muni$MM_casos = round(movavg(RJ_Muni$newCases, n = 7,type = "s"),0)
  
RJ_Muni$MM_obitos = round(movavg(RJ_Muni$newDeaths, n = 7,type = "s"),0)
  
RJ_Muni$MM_vacina = round(movavg(RJ_Muni$D1, n=7, type="s"),0)
  
RJ_Muni$MM_vacina2 = round(movavg(RJ_Muni$D2, n=7, type="s"),0)
  
RJ_Muni$MM_ocup_Cli = round(movavg(RJ_Muni$ocupacaoConfirmadoCli, n=7, type="s"),0)
  
RJ_Muni$MM_ocup_Uti = round(movavg(RJ_Muni$ocupacaoConfirmadoUti, n=7, type="s"),0)
...
```{r}
RJ_Muni$`Média móvel` <- round(RJ_Muni$MM_casos,0)
RJ_Muni$`Casos novos` <- RJ_Muni$newCases
RJ_Muni$Data <- RJ_Muni$date

g1 <- RJ_Muni |> ggplot(aes(x=Data, y=`Casos novos`))+ 
...
```{r}
RJ_Muni$`Média móvel` <- round(RJ_Muni$MM_obitos,0)
RJ_Muni$`Óbitos novos` <- RJ_Muni$newDeaths

g2 <- RJ_Muni |> ggplot(aes(x=Data, y=`Óbitos novos`))+ 
  geom_bar(stat="identity",
           fill="orange")+
  geom_line(aes(y=`Média móvel`),
            col="blue")+
...
```{r}
RJ_Muni$`Leitos clínicos ocupados` <- RJ_Muni$ocupacaoConfirmadoCli
RJ_Muni$`média móvel` <- RJ_Muni$MM_ocup_Cli

g7 <- RJ_Muni |> ggplot(aes(x=Data, y=`Leitos clínicos ocupados`))+ 
  geom_bar(stat="identity",
           fill="orange")+
  geom_line(aes(y=`média móvel`),
            col="blue")+
...
```{r}
RJ_Muni$`Leitos UTI ocupados` <- RJ_Muni$ocupacaoConfirmadoUti
RJ_Muni$`média móvel` <- RJ_Muni$MM_ocup_Uti

g8 <- RJ_Muni |> ggplot(aes(x=Data, y=`Leitos UTI ocupados`))+
  geom_bar(stat="identity",
           fill="orange")+
  geom_line(aes(y=`média móvel`),
            col="blue")+
...
```{r}
RJ_Muni$`Primeira dose` <- RJ_Muni$D1
RJ_Muni$`Segunda dose ou dose única` <- RJ_Muni$D2
...
```{r}
RJ_Muni$`Vacinas aplicadas` <- RJ_Muni$D1
...
```{r}
RJ_Muni$`Vacinas aplicadas` <- RJ_Muni$D2
...
```{r}
RJ_Muni$`Vacinas aplicadas` <- RJ_Muni$D3
...
for (i in 1:length(RJ_Muni$D1)){
  if(is.na(RJ_Muni$D1[i]==T)) RJ_Muni$D1[i]="Nao informado"
}
RJ_Muni= dplyr::filter(RJ_Muni, D1!="Nao informado")
RJ_Muni$D1= as.numeric(RJ_Muni$D1)

RJ_Muni$D1_AC= cumsum(RJ_Muni$D1)
RJ_Muni$D2_AC= cumsum(RJ_Muni$D2)
RJ_Muni$D3_AC= cumsum(RJ_Muni$D3)

RJ_Muni$D2_AC= as.character(RJ_Muni$D2_AC)
for (i in 1:length(RJ_Muni$D2_AC)){
  if((RJ_Muni$date[i]<"2021-01-17")) RJ_Muni$D2_AC[i]=NA
}
RJ_Muni$D2_AC= as.numeric(RJ_Muni$D2_AC)

RJ_Muni$D3_AC= as.character(RJ_Muni$D3_AC)
for (i in 1:length(RJ_Muni$D3_AC)){
  if((RJ_Muni$date[i]<"2021-08-03")) RJ_Muni$D3_AC[i]=NA
}
RJ_Muni$D3_AC= as.numeric(RJ_Muni$D3_AC)
...
```{r }
RJ_Muni$`Aplicações primeira dose` <- RJ_Muni$D1_AC
RJ_Muni$`Aplicações segunda dose ou dose única` <- RJ_Muni$D2_AC
RJ_Muni$`Aplicações terceira dose` <- RJ_Muni$D3_AC
...
#rm(BaseFinalRJ)
#rm(dados)
#RJ_Total$Populacao = as.numeric(as.character(RJ_Total$Populacao))
...
RJ_Total$Data <- ymd(RJ_Total$Data)
aux_data = RJ_Total |> 
  distinct(Data)

MM_casos = matrix(NA, ncol = dim(aux_nome)[1], nrow = max(table(RJ_Total$Municipios)))
MM_casos_max = matrix(NA, ncol = dim(aux_nome)[1], nrow = max(table(RJ_Total$Municipios)))
MM_obitos = matrix(NA, ncol = dim(aux_nome)[1], nrow = max(table(RJ_Total$Municipios)))
MM_obitos_max = matrix(NA, ncol = dim(aux_nome)[1], nrow = max(table(RJ_Total$Municipios)))
MM_tx_casos = matrix(NA, ncol = dim(aux_nome)[1], nrow = max(table(RJ_Total$Municipios)))
MM_tx_obitos = matrix(NA, ncol = dim(aux_nome)[1], nrow = max(table(RJ_Total$Municipios)))
casos_max = matrix(NA, ncol = dim(aux_nome)[1], nrow = max(table(RJ_Total$Municipios)))
obitos_max = matrix(NA, ncol = dim(aux_nome)[1], nrow = max(table(RJ_Total$Municipios)))
tx_casos_max = matrix(NA, ncol = dim(aux_nome)[1], nrow = max(table(RJ_Total$Municipios)))
tx_obitos_max = matrix(NA, ncol = dim(aux_nome)[1], nrow = max(table(RJ_Total$Municipios)))
#prop_CasosNovos = matrix(NA, ncol = dim(aux_nome)[1], nrow = max(table(RJ_Total$Municipios)))
#prop_ObitosNovos = matrix(NA, ncol = dim(aux_nome)[1], nrow = max(table(RJ_Total$Municipios)))
#total_casos_periodo1 = matrix(NA, ncol = dim(aux_nome)[1], nrow = max(table(RJ_Total$Municipios)))

#Periodo1 = os 7 dias mais recentes
#periodo1 = c(max(RJ_Total$Data, na.rm = T),max(RJ_Total$Data, na.rm = T) - 6) 
...
for(i in aux_nome$Municipios){
  ...
  MM_casos_max[,j]=rep(max(MM_casos[,j],na.rm = TRUE),max(table(RJ_Total$Municipios)))
  ...
  #Maximo dos casos
  casos_max[,j] = rep(max(base_aux$CasosAcumulados), max(table(RJ_Total$Municipios)))
  
  #Maximo das taxas de casos
  tx_casos_max[,j] = rep(max(base_aux$tx_casos_mil_diaria,na.rm = T),
                         max(table(RJ_Total$Municipios)))
  ...
  #prop_CasosNovos[,j] = rep(round(prop_casos,2), max(table(RJ_Total$Municipios)))
  
  #total_casos_periodo1[,j] = rep(casosNovos1,  max(table(RJ_Total$Municipios)))
  ...
  #Maximo dos obitos
  obitos_max[,j] = rep(max(base_aux$ObitosAcumulados), max(table(RJ_Total$Municipios)))
  ...
  #prop_ObitosNovos[,j] = rep(round(prop_obitos,2), max(table(RJ_Total$Municipios)))
...
#Adicionando as bases à base RJ_Total

#Casos maximos e taxa maxima de casos
RJ_Total = left_join(RJ_Total, casos_aux_geral_max, by = c("Data", "Municipios"))
RJ_Total = left_join(RJ_Total, tx_casos_aux_geral_max, by = c("Data", "Municipios"))

# Media movel dos casos e media movel ponderada
RJ_Total = left_join(RJ_Total,MM_aux_geral_casos, by = c("Data", "Municipios", "MM_casos"))
RJ_Total = left_join(RJ_Total,MM_aux_geral_casos_max, by = c("Data", "Municipios"))
RJ_Total$MM_casos_pond = RJ_Total$MM_casos / RJ_Total$MM_casos_max

#Media Movel das taxas de casos 
RJ_Total = left_join(RJ_Total,MM_aux_geral_tx_casos, by = c("Data", "Municipios"))


#obitos maximos e taxa maxima de obitos
RJ_Total = left_join(RJ_Total,obitos_aux_geral_max, by = c("Data", "Municipios"))
RJ_Total = left_join(RJ_Total,tx_obitos_aux_geral_max, by = c("Data", "Municipios"))

# Media movel dos obitos e media movel ponderada
RJ_Total = left_join(RJ_Total,MM_aux_geral_obitos, by = c("Data", "Municipios", "MM_obitos"))
RJ_Total = left_join(RJ_Total,MM_aux_geral_obitos_max, by = c("Data", "Municipios"))
RJ_Total$MM_obitos_pond = RJ_Total$MM_obitos / RJ_Total$MM_obitos_max

#Media Movel das taxas de obitos 
RJ_Total = left_join(RJ_Total,MM_aux_geral_tx_obitos, by = c("Data", "Municipios"))
...
#RJ_Total = left_join(RJ_Total, MM_final3, by = c("Data","Municipios"))
#RJ_Total = left_join(RJ_Total, prop_aux_geral_casosNovos, by = c("Data", "Municipios"))
#RJ_Total = left_join(RJ_Total, total_aux_geral_casos_periodo1, by = c("Data", "Municipios"))
#RJ_Total = left_join(RJ_Total, prop_aux_geral_obitosNovos, by = c("Data", "Municipios"))
...
RJ_Total = left_join(RJ_Total, primeiro_caso_RJ, by = "Municipios")

#Acrescentando a data do primeiro obito por RJ na base
RJ_Total = left_join(RJ_Total, primeiro_obito_RJ, by = "Municipios")
...
RJ_Total$num_dias_caso[RJ_Total$num_dias_caso <= 0] = NA
RJ_Total$num_dias_obito[RJ_Total$num_dias_obito <= 0] = NA

#Calculando letalidade e taxa por 100000 habitantes

#Ultima data de coleta de dados
data_hoje = paste(day(max(RJ_Total$Data)),"/",month(max(RJ_Total$Data)),"/",year(max(RJ_Total$Data)),sep = "")
...
RJ_Total2$D1= as.character(RJ_Total2$D1)
for (i in 1:length(RJ_Total2$D1)){
  if(is.na(RJ_Total2$D1[i]==T)) RJ_Total2$D1[i]=0
}
RJ_Total2$D1= as.numeric(RJ_Total2$D1)

for (i in 1:length(RJ_Total2$D2)){
  if(is.na(RJ_Total2$D2[i]==T)) RJ_Total2$D2[i]=0
}
RJ_Total2$D2= as.numeric(RJ_Total2$D2)

for (i in 1:length(RJ_Total2$D3)){
  if(is.na(RJ_Total2$D3[i]==T)) RJ_Total2$D3[i]=0
}
RJ_Total2$D3= as.numeric(RJ_Total2$D3)

#Data_hoje = max(RJ_Total$Data)
