...
download.file(url = "http://sistemas.saude.rj.gov.br/tabnetbd/csv/pni_covid16348180249.xlsx", 
...
download.file(url = "http://sistemas.saude.rj.gov.br/tabnetbd/csv/pni_covid16348176183.xlsx", 
              destfile = temp, mode="wb")
...
download.file(url = "http://sistemas.saude.rj.gov.br/tabnetbd/csv/pni_covid16348180045.xlsx", 
              destfile = temp, mode="wb")
...
casosRJ_final0 = casosRJ_final

casosRJ_final0 = casosRJ_final0 |> 
  group_by(date) |> 
...
casosRJ_final0$MM_casos = round(movavg(casosRJ_final0$newCases, n = 7,type = "s"),0)
  
casosRJ_final0$MM_obitos = round(movavg(casosRJ_final0$newDeaths, n = 7,type = "s"),0)
  
casosRJ_final0$MM_vacina = round(movavg(casosRJ_final0$D1, n=7, type="s"),0)
  
casosRJ_final0$MM_vacina2 = round(movavg(casosRJ_final0$D2, n=7, type="s"),0)
  
casosRJ_final0$MM_ocup_Cli = round(movavg(casosRJ_final0$ocupacaoConfirmadoCli, n=7, type="s"),0)
  
casosRJ_final0$MM_ocup_Uti = round(movavg(casosRJ_final0$ocupacaoConfirmadoUti, n=7, type="s"),0)
...
```{r}
casosRJ_final0$`Média móvel` <- round(casosRJ_final0$MM_casos,0)
casosRJ_final0$`Casos novos` <- casosRJ_final0$newCases
casosRJ_final0$Data <- casosRJ_final0$date

g1 <- casosRJ_final0 |> ggplot(aes(x=Data, y=`Casos novos`))+ 
...
```{r}
casosRJ_final0$`Média móvel` <- round(casosRJ_final0$MM_obitos,0)
casosRJ_final0$`Óbitos novos` <- casosRJ_final0$newDeaths

g2 <- casosRJ_final0 |> ggplot(aes(x=Data, y=`Óbitos novos`))+ 
  geom_bar(stat="identity",
           fill="orange")+
  geom_line(aes(y=`Média móvel`),
            col="blue")+
...
```{r}
casosRJ_final0$`Leitos clínicos ocupados` <- casosRJ_final0$ocupacaoConfirmadoCli
casosRJ_final0$`média móvel` <- casosRJ_final0$MM_ocup_Cli

g7 <- casosRJ_final0 |> ggplot(aes(x=Data, y=`Leitos clínicos ocupados`))+ 
  geom_bar(stat="identity",
           fill="orange")+
  geom_line(aes(y=`média móvel`),
            col="blue")+
...
```{r}
casosRJ_final0$`Leitos UTI ocupados` <- casosRJ_final0$ocupacaoConfirmadoUti
casosRJ_final0$`média móvel` <- casosRJ_final0$MM_ocup_Uti

g8 <- casosRJ_final0 |> ggplot(aes(x=Data, y=`Leitos UTI ocupados`))+
  geom_bar(stat="identity",
           fill="orange")+
  geom_line(aes(y=`média móvel`),
            col="blue")+
...
```{r}
casosRJ_final0$`Primeira dose` <- casosRJ_final0$D1
casosRJ_final0$`Segunda dose ou dose única` <- casosRJ_final0$D2
...
```{r}
casosRJ_final0$`Vacinas aplicadas` <- casosRJ_final0$D1
...
```{r}
casosRJ_final0$`Vacinas aplicadas` <- casosRJ_final0$D2
...
```{r}
casosRJ_final0$`Vacinas aplicadas` <- casosRJ_final0$D3
...
#  arrange(ibgeID) |> 
#  select(ibgeID, populacao)
casosRJ_final0$populacao= 17463349


#Acumulado das vacinas aplicadas
casosRJ_final0= casosRJ_final0 |> 
  filter(date>= "2021-01-01")
casosRJ_final0$D1= as.character(casosRJ_final0$D1)
for (i in 1:length(casosRJ_final0$D1)){
  if(is.na(casosRJ_final0$D1[i]==T)) casosRJ_final0$D1[i]="Nao informado"
}
casosRJ_final0= dplyr::filter(casosRJ_final0, D1!="Nao informado")
casosRJ_final0$D1= as.numeric(casosRJ_final0$D1)

casosRJ_final0$D1_AC= cumsum(casosRJ_final0$D1)
casosRJ_final0$D2_AC= cumsum(casosRJ_final0$D2)
casosRJ_final0$D3_AC= cumsum(casosRJ_final0$D3)

casosRJ_final0$D2_AC= as.character(casosRJ_final0$D2_AC)
for (i in 1:length(casosRJ_final0$D2_AC)){
  if((casosRJ_final0$date[i]<"2021-01-17")) casosRJ_final0$D2_AC[i]=NA
}
casosRJ_final0$D2_AC= as.numeric(casosRJ_final0$D2_AC)

casosRJ_final0$D3_AC= as.character(casosRJ_final0$D3_AC)
for (i in 1:length(casosRJ_final0$D3_AC)){
  if((casosRJ_final0$date[i]<"2021-08-03")) casosRJ_final0$D3_AC[i]=NA
}
casosRJ_final0$D3_AC= as.numeric(casosRJ_final0$D3_AC)
...
```{r }
casosRJ_final0$`Aplicações primeira dose` <- casosRJ_final0$D1_AC
casosRJ_final0$`Aplicações segunda dose ou dose única` <- casosRJ_final0$D2_AC
casosRJ_final0$`Aplicações terceira dose` <- casosRJ_final0$D3_AC
...
```{r fig.align='center', echo=FALSE, message=FALSE, warning=FALSE, fig.width = 10}
#load("casosRJ_final.rdata")
casosRJ_final2= casosRJ_final |>
  select(Data, Municipios, D1, D2, D3) 

casosRJ_final2$D1= as.character(casosRJ_final2$D1)
for (i in 1:length(casosRJ_final2$D1)){
  if(is.na(casosRJ_final2$D1[i]==T)) casosRJ_final2$D1[i]=0
}
casosRJ_final2$D1= as.numeric(casosRJ_final2$D1)

for (i in 1:length(casosRJ_final2$D2)){
  if(is.na(casosRJ_final2$D2[i]==T)) casosRJ_final2$D2[i]=0
}
casosRJ_final2$D2= as.numeric(casosRJ_final2$D2)

for (i in 1:length(casosRJ_final2$D3)){
  if(is.na(casosRJ_final2$D3[i]==T)) casosRJ_final2$D3[i]=0
}
casosRJ_final2$D3= as.numeric(casosRJ_final2$D3)

#Data_hoje = max(casosRJ_final$Data)

casosRJ_final2 = casosRJ_final2 |> 
  group_by(Municipios) |> 
