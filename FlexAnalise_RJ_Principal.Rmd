...
download.file(url = "http://sistemas.saude.rj.gov.br/tabnetbd/csv/pni_covid16344930335.xlsx", 
...
download.file(url = "http://sistemas.saude.rj.gov.br/tabnetbd/csv/pni_covid16344930371.xlsx", 
              destfile = temp, mode="wb")
...
download.file(url = "http://sistemas.saude.rj.gov.br/tabnetbd/csv/pni_covid16344930622.xlsx", 
              destfile = temp, mode="wb")
...
```{r}
casosRJ_final$`Média móvel` <- round(casosRJ_final$MM_casos,0)
casosRJ_final$`Casos novos` <- casosRJ_final$newCases

g1 <- casosRJ_final |> ggplot(aes(x=date, y=`Casos novos`))+ 
...
```{r}
casosRJ_final$`Média móvel` <- round(casosRJ_final$MM_obitos,0)
casosRJ_final$`Óbitos novos` <- casosRJ_final$newDeaths

g2 <- casosRJ_final |> ggplot(aes(x=date, y=`Óbitos novos`))+ 
  geom_bar(stat="identity",
           fill="orange")+
  geom_line(aes(y=`Média móvel`),
            col="blue")+
...
```{r}
casosRJ_final$`Leitos clínicos ocupados` <- casosRJ_final$ocupacaoConfirmadoCli
casosRJ_final$`média móvel` <- casosRJ_final$MM_ocup_Cli

g7 <- casosRJ_final |> ggplot(aes(x=date, y=`Leitos clínicos ocupados`))+ 
  geom_bar(stat="identity",
           fill="orange")+
  geom_line(aes(y=`média móvel`),
            col="blue")+
...
```{r}
casosRJ_final$`Leitos UTI ocupados` <- casosRJ_final$ocupacaoConfirmadoUti
casosRJ_final$`média móvel` <- casosRJ_final$MM_ocup_Uti

g8 <- casosRJ_final |> ggplot(aes(x=date, y=`Leitos UTI ocupados`))+
  geom_bar(stat="identity",
           fill="orange")+
  geom_line(aes(y=`média móvel`),
            col="blue")+
...
```{r}
casosRJ_final$`Primeira dose` <- casosRJ_final$D1
casosRJ_final$`Segunda dose ou dose única` <- casosRJ_final$D2

g3 <- casosRJ_final |> 
  filter(date>="2021-01-01") |> 
  ggplot(aes(x=date, y=`Primeira dose`))+ 
  geom_bar(stat="identity",
           fill="orange")+
...
```{r}
casosRJ_final$`Vacinas aplicadas` <- casosRJ_final$D1

g4 <- casosRJ_final |> 
  filter(date>="2021-01-01") |> 
  ggplot(aes(x=date, y=`Vacinas aplicadas`))+ 
  geom_bar(stat="identity",
           fill="red")+
  scale_x_date(date_breaks = "month",
               date_labels = "%b/%y")+
...
```{r}
casosRJ_final$`Vacinas aplicadas` <- casosRJ_final$D2
...
```{r}
casosRJ_final$`Vacinas aplicadas` <- casosRJ_final$D3
...
for (i in 1:length(casosRJ_final2$D1)){
  if(is.na(casosRJ_final2$D1[i]==T)) casosRJ_final2$D1[i]="Nao informado"
}
casosRJ_final2= dplyr::filter(casosRJ_final2, D1!="Nao informado")
casosRJ_final2$D1= as.numeric(casosRJ_final2$D1)

casosRJ_final2$D1_AC= cumsum(casosRJ_final2$D1)
casosRJ_final2$D2_AC= cumsum(casosRJ_final2$D2)
casosRJ_final2$D3_AC= cumsum(casosRJ_final2$D3)

casosRJ_final2$D2_AC= as.character(casosRJ_final2$D2_AC)
for (i in 1:length(casosRJ_final2$D2_AC)){
  if((casosRJ_final2$date[i]<"2021-01-17")) casosRJ_final2$D2_AC[i]=NA
}
casosRJ_final2$D2_AC= as.numeric(casosRJ_final2$D2_AC)

casosRJ_final2$D3_AC= as.character(casosRJ_final2$D3_AC)
for (i in 1:length(casosRJ_final2$D3_AC)){
  if((casosRJ_final2$date[i]<"2021-08-03")) casosRJ_final2$D3_AC[i]=NA
}
casosRJ_final2$D3_AC= as.numeric(casosRJ_final2$D3_AC)
...
```{r }
casosRJ_final2$`Aplicações primeira dose` <- casosRJ_final2$D1_AC
casosRJ_final2$`Aplicações segunda dose ou dose única` <- casosRJ_final2$D2_AC
casosRJ_final2$`Aplicações terceira dose` <- casosRJ_final2$D3_AC
